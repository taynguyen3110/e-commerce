...
const ButtonGroup = ({ next, previous, goToSlide, ...rest }: any) => {
    ...
    return (
        <div className='carousel-button-group flex flex-row-reverse absolute xl:right-24 md:right-12 right-4 md:top-[-85px] -top-11 gap-5'>
            <a className='cursor-pointer' onClick={() => next()}><img src={rightArrowIcon} /></a>
            <a className='cursor-pointer' onClick={() => previous()}><img src={leftArrowIcon} /></a>
        </div>
...
export const CarouselSection = () => {
    ...
    return (
        ...
        >
            {
                reviews.map(r =>
                    <ReviewCard review={r} carousel={true} />
                )
            }
        </Carousel>
