      ...
      <div className="container md:mx-auto md:px-0 px-4 nav-bar flex md:gap-10 justify-between text-center my-4">
        <div className="flex lg:gap-10 md:gap-6 items-center md:w-2/3">
          ...
          <div className="nav-logo relative">
            <p className="relative md:-top-1 md:text-[32px] -top-[2px] text-2xl md:ml-0 ml-4">
              <a href="/">Shop.co</a>
            </p>
          </div>
          <div className="nav-menu lg:gap-6 gap-4 h-12 justify-stretch items-center md:flex hidden">
            <Dropdown trigger={<button>Shop</button>}>
              <div className="flex flex-col items-start w-40 z-50">
                <a
                  className="w-full py-3 hover:bg-bghighlight hover:text-highlight bg-white rounded-md"
                  href="/category/Calvin Klein"
                >
                  Calvin Klein
                </a>
                <a
            ...
            <div className="text-nowrap">
              <a href="/category/On Sale">On Sale</a>
            </div>
            <div className="text-nowrap">
        ...
        <div className="flex md:gap-x-10 lg:w-full md:w-2/3 w-1/2 justify-end items-center gap-3.5">
          <div className="min-[480px]:hidden">
            <Dropdown
              trigger={<img src={searchIcon} alt="" />}
              fullWidth={true}
              align="right"
              offset={88}
            >
          ...
          <div className="nav-btns flex items-center justify-between gap-3">
            <div className="relative">
              <a className="cursor-pointer" href="/cart">
                ...
                <span className="absolute bottom-0 left-4 rounded-full text-white text-[10px] bg-red-600 z-30 w-[18px] h-[18px] pt-[2px]">
                  {cartItems.length}
                </span>
              </a>
            </div>
            {!user ? (
              <i
                className="bx bx-user-circle text-2xl font-bold cursor-pointer"
                onClick={() => {
                  displayLogin();
                }}
              ></i>
              ...
              >
                <motion.div
                  style={{ x: 20 }}
                  animate={{ x: 0 }}
                  className="mix-w-52 p-4"
                >
                  <h3>PROFILE</h3>
                  <hr className="my-3" />
                  <div className="flex flex-col items-start text-nowrap text-sm gap-1">
                    <span>
                      Email: <p className="inline">{user.email}</p>
                    </span>
                    <span>
                      Joined at:{" "}
                      <p className="inline">{user.metadata.creationTime}</p>
                    </span>
                    <span>
                      Last Login:{" "}
                      <p className="inline">{user.metadata.lastSignInTime}</p>
                    </span>
                  </div>
                  <hr className="my-3" />
                  <Button
