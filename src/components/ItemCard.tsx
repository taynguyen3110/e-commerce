...
export const ItemCard = ({ product, itemEachRows }: ItemCardProps) => {
  ...
  if (product) {
    return (
      <div
        className="flex flex-col gap-[6px] md:mb-0 mb-5"
      ...
      >
        ...
        <div>
          <a href={`/product/${product.id}`}>
            <p className="lg:text-xl text-base font-bold cursor-pointer truncate">
              {product.name}
            </p>
          </a>
        </div>
        ...
        <div>
          <div className="flex items-center gap-[10px] lg:text-2xl text-xl font-bold select-none">
            <div>
              <span>${calSalePrice(product)}</span>
            </div>
            {product.sale !== 0 ? (
              <>
                ...
                <div className="lg:text-xs text-[10px] flex items-center md:px-[14px] md:py-[6px] px-1 h-5 text-[#FF3333] bg-opacity-10 bg-[#FF3333] rounded-full">
